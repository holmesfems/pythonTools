...
def doplot(plots,option):
    arss = []
    subplots=None
    if not option['subplots'] is None:
        subplots=str_to_intarr(option['subplots'])
    for plot in plots:
    ...
    allplot=subplots[0]*subplots[1]
    ploted=1
    for item in arss:
        if ploted>allplot:
            break
        ax=fig.add_subplot(subplots[0],subplots[1],ploted)
        if option['xlabel'] is None:
            option['xlabel']=item[0][1]
        ...
        ax.set_title(option['title'][ploted-1])
        yplot=None
        ...
        for y in item[1:]:
            ...
            if width_type==3:
                ...
                lw=lws[0]
            ax.plot(item[0][0],y[0],linewidth=lw,linestyle='-',label=label)
        ploted+=1
    plt.legend()
...
def main(argv):
    ...
    #'xticks_minor':None, #not available
    #'yticks_minor':None,
    if len(argv)>1:
        plots=[]
        is_readfile=False
