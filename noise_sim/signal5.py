...
def gaussianFit(array):
    ...
    amp=s/(2*math.pi*sig2)**0.5
    Y=[]
    for i in range(0,len(array)):
        Y.append(gaussian(i,mu=mu,sig2=sig2,amp=amp)**0.5)
...
def gaussianFit2(array):
    ...
    Xt=matrix.turn(X)
    #print X[0]
    M=matrix.matmul(Xt,X)
    K=matrix.matmul(Xt,Y)
    A=matrix.matmul(matrix.matinv(M),K)
    A=matrix.turn(A)[0]
    sig2=-1.0/A[2]
    mu=A[1]*sig2/2.0
    P=math.exp((A[0]+mu*mu/sig2)/2.0)
    ret=[]
    for i in range(0,len(array)):
        ret.append(gaussian(i,mu=mu,sig2=sig2,amp=P))
    return ret
