...
//token to represent a tree by a name
class AnalyzeToken
{
    public:
        std::string name;
...
class AnalyzeTokenVectorDict
{
    ...
    public:
        ...
        std::vector<AnalyzeTokenVectorMap>& getDict(){return _dict;}
    ...
    switch(cmd2.c_str()[0])
    {
        ...
        default:
        {
            try{
                ...
                std::cout << doCalculate(cmd2) << std::endl;
            }
...
{
    _phyTypeDict=new PhyTypeDict();
    std::string strmass="mass",strkg="kg";
    PhyBaseUnitDict *mass=new PhyBaseUnitDict(strmass);
    mass->addUnit(new PhyBaseUnit(strkg,strmass,1.0));
    std::cout<<mass->changeDefault(0)<<std::endl;
    _phyTypeDict->addDict(mass);
    PhyValue a(1.0,PhyUnit(strmass));
    std::cout << "created!" << std::endl;
    std::cout << a.str() << std::endl;
    PhyValue b(2.0,PhyUnit(strmass));
